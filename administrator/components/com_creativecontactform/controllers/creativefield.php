...
class CreativeContactFormControllerCreativeField extends JControllerForm
{
	...
	function saveField() {
		...
		if ($msg_string == 'no') {
			...
			$msg_type = 'message';
		} else if($msg_string == 'license') {
			$msg = 'Please upgrade to commercial version to use this field type';
			$msg_type = 'error';
		}
		else {
		...
		if(($_REQUEST['task'] == 'apply' || $_REQUEST['task'] == 'creativefield.apply') && $id != 0)
