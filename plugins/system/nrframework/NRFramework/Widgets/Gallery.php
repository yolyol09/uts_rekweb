...
class Gallery extends Widget
{
	...
	private function setCSSVariables()
	{
		...
		{
			$css = array_merge_recursive($css, $gap);
		}

		if ($this->options['style'] === 'justified')
		{
			$css['desktop'][] = '--justified-item-height: ' . $this->options['justified_item_height'] . 'px';
		}
		else
		{
			if ($columns = \NRFramework\Helpers\Controls\Responsive::getResponsiveControlValue($this->options['columns'], '--columns'))
			{
				$css = array_merge_recursive($css, $columns);
			}
		}

		if (!$css = \NRFramework\Helpers\Responsive::renderResponsiveCSS($css, '.nrf-widget.' . $this->options['id']))
		{
			return;
		}
