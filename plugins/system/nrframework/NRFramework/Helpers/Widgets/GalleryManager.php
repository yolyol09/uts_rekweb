	...
	*/
	...
	public static function moveTempItemsToDestination($value, $field, $destination_folder)
	{
		...
		foreach ($items as $key => &$item)
		{
			...
			if (Functions::startsWith($item['thumbnail'], self::$temp_folder) && file_exists($thumbnail_path))
			{
				// Move thumbnail
			...
			if (Functions::startsWith($item['image'], self::$temp_folder) && file_exists($image_path))
			{
				// Move image
